...
\hypersetup{
  ...
  pdftitle={Boundaries of Retail Quantitative Trading on Prediction Markets: A Multi-Mechanism Empirical Retrospective}
}

\title{Boundaries of Retail Quantitative Trading on Prediction Markets:\\
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.105\textwidth}>{\raggedright\arraybackslash}p{0.135\textwidth}>{\raggedright\arraybackslash}p{0.145\textwidth}>{\raggedright\arraybackslash}p{0.135\textwidth}>{\raggedright\arraybackslash}p{0.160\textwidth}>{\raggedright\arraybackslash}p{0.120\textwidth}>{\raggedright\arraybackslash}p{0.160\textwidth}@{}}
\toprule
...
LiveTradeBench (\cite{Yu2025LiveSeek}) & Live market data for real-world alpha-seeking evaluation. & Whether LLM agents can seek trading alpha in live settings. & Partial: failed predictions can be scored, but the paper is not built around retained route refusals. & Moves evaluation from static finance QA toward live trading substrates. & External benchmark setting rather than Abel AI Lab's single-operator archive. & Quantitative outcome is benchmark performance, not failure-preserved route attrition. \\
...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
  route/.style={rectangle, rounded corners=2pt, draw=black!60, very thick, align=center, minimum height=2.0cm, text width=3.2cm, fill=black!5},
  gate/.style={trapezium, trapezium left angle=70, trapezium right angle=110, draw=black!60, very thick, align=center, minimum height=2.2cm, text width=3.1cm, fill=blue!8},
  outcome/.style={rectangle, rounded corners=2pt, draw=black!65, very thick, align=center, minimum height=1.1cm, text width=5.6cm, fill=red!8},
  arrow/.style={-{Latex[length=3mm]}, thick, draw=black!65}
]
\node[route] (routes) {228 alternative\\routes proposed};
\node[gate, fill=green!10, right=0.7cm of routes] (sourceability) {Sourceability\\gate\\45 selected};
\node[gate, fill=yellow!18, right=0.7cm of sourceability] (fillability) {Fillability\\gate\\current book and venue access};
\node[gate, fill=orange!16, right=0.7cm of fillability] (capacity) {Capacity\\gate\\\$5.34 $\rightarrow$ \$200--500 $\rightarrow$ \$3--6k floor};
\node[gate, fill=purple!10, right=0.7cm of capacity] (anchor) {Anchor stability\\gate\\cuts LLM-prior routes near visible quotes};
\node[outcome, below=1.0cm of capacity, xshift=2.0cm] (survivors) {0 sustainable\\\textgreater{}\$1k/month routes survive};

\draw[arrow] (routes) -- (sourceability);
\draw[arrow] (sourceability) -- (fillability);
\draw[arrow] (fillability) -- (capacity);
\draw[arrow] (capacity) -- (anchor);
\draw[arrow] (anchor.south) |- (survivors.east);
\end{tikzpicture}%
}
\caption{Four-gate filter for retail LLM-PM trading routes. Of 228 proposed routes, 0 surveyed campaigns produced sustainable \textgreater{}\$1k/month edge after all gates apply.}
...
\begin{tikzpicture}[x=0.045cm, y=0.9cm]
\draw[black!45] (0,0) -- (225,0);
\foreach \x/\lab in {0/0,45/45,90/90,135/135,180/180,225/225} {
  \draw[black!45] (\x,-0.08) -- (\x,0.08);
  \node[anchor=north, font=\small] at (\x,-0.12) {\lab};
}
\node[anchor=east, font=\small] at (-5,4.1) {Selected};
\draw[fill=green!45, draw=green!45!black, very thick] (0,3.85) rectangle (45,4.35);
\node[anchor=west, font=\small] at (50,4.1) {45 routes (20.0\%)};

\node[anchor=east, font=\small] at (-5,3.1) {Rejected};
\draw[fill=red!38, draw=red!55!black, very thick] (0,2.85) rectangle (135,3.35);
\node[anchor=west, font=\small] at (140,3.1) {135 routes (60.0\%)};

\node[anchor=east, font=\small] at (-5,2.1) {Deferred};
\draw[fill=yellow!65!orange, draw=yellow!45!black, very thick] (0,1.85) rectangle (45,2.35);
\node[anchor=west, font=\small] at (50,2.1) {45 routes (20.0\%)};

\node[anchor=east, font=\small] at (-5,1.1) {Promoted};
\draw[fill=blue!40, draw=blue!60!black, very thick] (0,0.85) rectangle (79,1.35);
\node[anchor=west, font=\small] at (84,1.1) {79 routes promoted to subsequent rounds (overlapping)};
\end{tikzpicture}%
}
\caption{Recovered alternative-routes registry: 225 routes total, 45 selected (20.0\%), 135 rejected (60.0\%), 45 deferred (20.0\%), 79 promoted to subsequent rounds (overlapping). The aggregate campaign table reports a slightly larger denominator (228 proposed, 82 promoted) because some early-round routes were not preserved verbatim in the recovered registry; \S3.1 separates the two counts.}
...
The quote bank anchors the saturation claim at the places where the routes actually broke: Q001 \texttt{abelian/runs/2026-05-03-0014-cron-fill-grounded-truth/round-1/peer-B.txt:L60-L60}; Q002 \texttt{abelian/runs/2026-05-03-0245-ws-cron-alignment/round-1/peer-A.txt:L39-L39}; Q003 \texttt{abelian/runs/2026-05-03-0245-ws-cron-alignment/round-1/peer-A.txt:L171-L171}; Q004 \texttt{abelian/runs/2026-05-03-0245-ws-cron-alignment/round-2/peer-B.txt:L19-L19}; Q005 \texttt{abelian/runs/2026-05-03-0245-ws-cron-alignment/round-2/peer-B.txt:L21-L21}.
...
\noindent evidence: \textit{``0/18 events were skipped on \texttt{rule\_reader\_confidence $<$ 80} --- every event scored 100. This is a ceiling-pinned signal''} --- \texttt{2026-05-04-llm-deploy-10k/round-1/peer-A.txt:L16-L16}

\noindent evidence: \textit{``Rubric is \texttt{25 $\times$ n\_extracted\_fields} and PM gamma metadata trivially has all 4 fields''} --- \texttt{2026-05-04-llm-deploy-10k/round-1/peer-A.txt:L39-L39}

\noindent evidence: \textit{``passed\_rule\_reader ($\geq$80 confidence) | 18 (100\%)''} --- \texttt{2026-05-04-llm-deploy-10k/COMPOUND\_DOC.md:L22-L22}

\noindent evidence: \textit{``18-event historical backtest shows -23\% net on \$4250 deployed with hit\_rate 41.2\%''} --- \texttt{2026-05-04-llm-deploy-10k/COMPOUND\_DOC.md:L4-L4}
...
Instance two is Path C, the EDGAR special-situations probe. R1 repeated the llm-deploy defect in a different mechanism: regulatory and consideration axes could auto-credit generic strings. The domain changed from Polymarket Gamma markets to SEC filings, but the same engineering mistake recurred. The rubric rewarded an answer-shaped field before it verified that the field meant what the trading route needed.

\noindent evidence: \textit{``R1 ships SAME ceiling-pin defect class as llm-deploy V3 (regulatory + consideration\_quant axes auto-credit on truthy-string)''} --- \texttt{2026-05-05-path-c-1week-probe/round-1/peer-A.txt:L8-L8}

\noindent evidence: \textit{``regulatory\_agency axis ceiling-pinned: \texttt{scoring.py:10-11} accepts ANY non-empty string''} --- \texttt{2026-05-05-path-c-1week-probe/round-1/peer-A.txt:L39-L39}

\noindent evidence: \textit{``consideration\_quant axis ceiling-pinned: \texttt{scoring.py:5} accepts ANY non-empty string''} --- \texttt{2026-05-05-path-c-1week-probe/round-1/peer-A.txt:L40-L40}
...
Path C R2 repaired the most obvious code-level auto-credit problem and then revealed the same defect class at the prompt layer. The LLM produced plausible allowlisted phrases such as "none required" and financing prose that passed the tighter rubric even when the cited filing chunks did not contain those strings. The gate moved from rubric-side slot filling to prompt-side plausible-string generation.

\noindent evidence: \textit{``the SOLE high-confidence row is a hallucination on 3 fields''} --- \texttt{2026-05-05-path-c-1week-probe/round-2/peer-A.txt:L8-L8}

\noindent evidence: \textit{``the LLM fabricated "none required" in the JSON output''} --- \texttt{2026-05-05-path-c-1week-probe/round-2/peer-A.txt:L41-L41}

\noindent evidence: \textit{``Quality-adjusted $\geq$80 rate = 0/18 today''} --- \texttt{2026-05-05-path-c-1week-probe/round-2/peer-A.txt:L63-L63}
...
\noindent evidence: \textit{``Engine prompt embeds market\_quote BEFORE asking for prior''} --- \texttt{2026-05-04-0125-llm-narrative-trade/round-1/peer-A.txt:L15-L15}

\noindent evidence: \textit{``the LLM is NOT citing the SOURCE BUNDLE. It's citing FACTS FROM TRAINING DATA''} --- \texttt{2026-05-04-0125-llm-narrative-trade/round-1/peer-A.txt:L55-L55}

\noindent evidence: \textit{``Signal: \textbf{7/8 events} show with-quote prior pulled toward market regardless of sign''} --- \texttt{2026-05-04-0125-llm-narrative-trade/round-2/peer-A.txt:L24-L24}

\noindent evidence: \textit{``Updated \texttt{with\_quote=False} prompt to explicitly prohibit market anchoring.''} --- \texttt{2026-05-04-0125-llm-narrative-trade/round-3/codex-implement.txt:L13015-L13015}

\noindent evidence: \textit{``events\_with\_24h\_pnl\_backfilled: \texttt{0}''} --- \texttt{2026-05-04-0125-llm-narrative-trade/round-3/codex-implement.txt:L13047-L13047}
...
\draw[black!50, -{Latex[length=2.5mm]}] (0,0) -- (12.4,0);
\foreach \x/\lab in {0/\$1,2.4/\$10,4.8/\$100,7.2/\$1k,9.6/\$10k,12/\$100k} {
  \draw[black!45] (\x,-0.08) -- (\x,0.08);
  \node[anchor=north, font=\small] at (\x,-0.15) {\lab};
  \draw[black!10] (\x,0.2) -- (\x,5.4);
}
\node[anchor=west, font=\small] at (0,5.65) {Displayed depth on log-scale dollar axis};

\node[anchor=east, align=right, text width=4.4cm, font=\small] at (-0.2,4.82) {crypto-pm-probe\\AXIS 2 fillability};
\draw[fill=teal!35, draw=black!55, very thick] (0,4.65) rectangle (1.75,4.99);
\node[anchor=west, align=left, text width=5.1cm, font=\small] at (1.90,4.82) {\$5.34};

\node[anchor=east, align=right, text width=4.4cm, font=\small] at (-0.2,3.82) {llm-narrative-trade\\single-row deployable};
\draw[fill=orange!35, draw=black!55, very thick] (0,3.65) rectangle (6.48,3.99);
\node[anchor=west, align=left, text width=5.1cm, font=\small] at (6.63,3.82) {\$200--\$500};

\node[anchor=east, align=right, text width=4.4cm, font=\small] at (-0.2,2.82) {llm-narrative-trade\\daily 30+ rows};
\draw[fill=orange!55, draw=black!55, very thick] (0,2.65) rectangle (9.07,2.99);
\node[anchor=west, align=left, text width=5.1cm, font=\small] at (9.22,2.82) {\$3,000--\$6,000};

\node[anchor=east, align=right, text width=4.4cm, font=\small] at (-0.2,1.82) {proposed 5-candidate\\Path C bandwidth};
\draw[fill=purple!30, draw=black!55, very thick] (0,1.65) rectangle (10.56,1.99);
\node[anchor=west, align=left, text width=5.1cm, font=\small] at (10.71,1.82) {\$25,000};

\node[anchor=east, align=right, text width=4.4cm, font=\small] at (-0.2,0.82) {pm-10k50k-profit\\shadow cumulative};
\draw[fill=blue!35, draw=black!55, very thick] (0,0.65) rectangle (11.84,0.99);
\node[anchor=west, align=left, text width=5.1cm, font=\small] at (11.99,0.82) {\$85,446.78};
\end{tikzpicture}%
}
\caption{Capacity ladder observed across retail prediction-market routes. The log-scale bars separate tiny fillability from single-row deployability, multi-row daily capacity, proposed bandwidth, and shadow-only cumulative frontier capacity.}
\label{fig:capacity-ladder}
\end{figure}

Crypto-pm-probe illustrates the small-cycle version. A route could look attractive on gross spread, but the bottleneck side of the book carried only a few dollars of fillable size. That is not a rounding issue. It changes the object being evaluated from a portfolio strategy to a market microstructure curiosity.

\noindent evidence: \textit{``11 contracts at NO best price $\approx$ \$0.485 = \~{}\$5.34 of fillable NO depth''} --- \texttt{2026-05-03-2025-crypto-pm-probe/round-2/peer-A.txt:L74-L74}

\noindent evidence: \textit{``AXIS 2 FAILS FILLABILITY --- the alleged 23.6\% spread is CAPACITY $\leq$ \$5.34''} --- \texttt{2026-05-03-2025-crypto-pm-probe/round-2/peer-A.txt:L74-L74}

\noindent evidence: \textit{``R3 must: (a) reject rows where min(inside\_size\_yes\_dollars, inside\_size\_no\_dollars) $<$ \$100''} --- \texttt{2026-05-03-2025-crypto-pm-probe/round-2/peer-A.txt:L75-L75}
...
llm-narrative-trade-0125 exposes the pre-PnL version. Peer review found a real CLOB book for one striking gap, but the immediately tradeable amount was still retail-small. The same paragraph that falsified the stale-Gamma hypothesis also warned that the deployable slice was limited without multi-day queueing or larger event selection.

\noindent evidence: \textit{``CLOB book: bid=0.51, ask=0.52, inside\_size\_yes=11130 USDC, inside\_size\_no=16052 USDC''} --- \texttt{2026-05-04-0125-llm-narrative-trade/round-2/peer-A.txt:L82-L82}

\noindent evidence: \textit{``at \$50-100 inside-size per fill before slippage past 0.52$\to$0.55 = \~{}\$200-500 deployable''} --- \texttt{2026-05-04-0125-llm-narrative-trade/round-2/peer-A.txt:L83-L83}

\noindent evidence: \textit{``At \$50-200 per-event inside-size and 30+ events for statistical power, daily deployable $\approx$ \$3-6k''} --- \texttt{2026-05-04-0125-llm-narrative-trade/round-2/peer-A.txt:L88-L88}
...
Bankruptcy-operator-0125 contributes a different capacity failure: access depth rather than book depth. The available public-scrape evidence did not expose enough claim-specific executable inventory. Aggregate pricing pages and plan snippets can support a desk memo, but a \$100k deployer needs class, minimum size, transfer mechanics, fees, and actual broker quotes.

\noindent evidence: \textit{``The claims\_log has only two rows.''} --- \texttt{2026-05-04-0125-bankruptcy-operator/round-1/peer-B.txt:L94-L95}

\noindent evidence: \textit{``the dataset is sparse''} --- \texttt{2026-05-04-0125-bankruptcy-operator/round-1/peer-B.txt:L97-L98}

\noindent evidence: \textit{``public web scraping does not expose enough live, actionable, claim-specific inventory for a \$100k deployer''} --- \texttt{2026-05-04-0125-bankruptcy-operator/round-2/peer-B.txt:L25-L25}
...
PM-10k50k-profit gives the most constructive counterexample. One round verified cumulative-to-50bps capacity of \$85,446.78 across four accepted candidates and treated that capacity as a frontier artifact rather than fantasy PnL. Later peer review verified that the frontier was shadow-only, that no live orders were created, and that deployable rows still needed current evidence and priors before paper entry.

\noindent evidence: \textit{``accepted\_capacity\_usd=85446.78 across 4 accepts (\~{}\$21k avg) is plausible for PM mid-tier liquidity''} --- \texttt{2026-05-04-1846-pm-10k50k-profit/round-1/peer-A.txt:L35-L35}

\noindent evidence: \textit{``capacity stats excluded from pool\_value\_delta\_usd --- prevents the "phantom pool growth from unfilled candidate depth" inflation class''} --- \texttt{2026-05-04-1846-pm-10k50k-profit/round-1/peer-A.txt:L36-L36}

\noindent evidence: \textit{``40 rows, 8 deployable candidates, 15 capacity-deployable rows, 30 required-prior-deployable rows, 0 positions appended, 0 live orders''} --- \texttt{2026-05-04-1846-pm-10k50k-profit/round-20/peer-B.txt:L17-L17}
...
The quote bank ties the anchoring constraint to specific prompt and prior failures: Q019 \texttt{abelian/runs/2026-05-03-1900-profit-search/round-1/peer-B.txt:L26-L26}; Q020 \texttt{abelian/runs/2026-05-04-0125-llm-narrative-trade/round-2/peer-A.txt:L15-L15}; Q021 \texttt{abelian/runs/2026-05-04-0125-llm-narrative-trade/round-2/peer-A.txt:L16-L16}; Q022 \texttt{abelian/runs/2026-05-04-0125-llm-narrative-trade/round-2/peer-A.txt:L17-L17}; Q023 \texttt{abelian/runs/2026-05-04-0125-llm-narrative-trade/round-2/peer-A.txt:L18-L18}.
...
\addplot[mark=square*, blue, thick] coordinates {(1,18) (2,3) (3,1) (4,0)};
\addlegendentry{Path C}
\addplot[mark=triangle*, red, thick, dashed] coordinates {(1,21) (2,6) (3,4) (4,3) (5,1)};
\addlegendentry{Paper 2}
\end{axis}
\end{tikzpicture}
\caption{Round-zero attack-reduction trajectory across two campaigns. Path C narrowed 18 attacks to 0 across 4 program-gate iterations; Paper 2 narrowed 21 to 1 across 5 iterations before R1 implementation.}
...
Cross-campaign aggregate file: \texttt{docs/research/paper-2-llm-extraction-and-pm-saturation/cross-campaign-aggregate.json}

Attack registry: \texttt{docs/research/paper-2-llm-extraction-and-pm-saturation/anc/attack-registry.jsonl}

Methodology self-attacks: \texttt{docs/research/paper-2-llm-extraction-and-pm-saturation/anc/methodology-self-attacks.jsonl}

Alternative-route registry: \texttt{docs/research/paper-2-llm-extraction-and-pm-saturation/anc/alternative-routes.jsonl}

Quote bank: \texttt{docs/research/paper-2-llm-extraction-and-pm-saturation/anc/quote-bank.jsonl}

MarketAnchor v0.1.2 cross-cite SHA256: 8968953708979dc2bd2915ec5c0a22c40e730c24d8134470739b7084bec5ca27
